...
class RouletteGame:
    ...
    def get_winners(self) -> Dict[int, Decimal]:
        ...
        # Find winners (players who chose the winning number)
        ...
        if winners:
            total_winning_bets = sum(self.player_bets[player_id] for player_id in winners)
            losing_bets = total_bets - total_winning_bets

            # Distribute losing bets proportionally among winners based on their bet sizes
            for player_id in winners:
                proportion = self.player_bets[player_id] / total_winning_bets
                winners[player_id] += losing_bets * proportion

        logger.info(f"Winners on table {self.table_id}: {winners}")
        return winners